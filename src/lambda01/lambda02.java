...
public class lambda02 {
    ...
    public static void main(String[] args) {
        ...
        List<Integer> l=new ArrayList<>(Arrays.asList(12,7,8,10,6,8,9,2));
        elemanYazdir(l);
             System.out.println();
        ciftSayiYazdir(l);
            System.out.println();
    ...
    public static void elemanYazdir(List<Integer> l) {
        l.stream().forEach(metodLambda2::bosluklaYazdir);//bizim metotla yazdik
   ...
   // }
    ...
    public static List farkliTersSirala(List<Integer> l) {
      return   l.stream().distinct().filter(t->t>5 && t%2==0).map(t->t/2).sorted(Comparator.reverseOrder()).collect(Collectors.toList());//collect terminal elemani
    }
    //8List teki en büyük elemanı bul.(Use Method Reference)
    public static void maxDeger(List<Integer> l) {
        System.out.println(l.stream().reduce(Integer.MAX_VALUE,Math::max));//dizideki en dusuk sayi
        System.out.println(l.stream().reduce(Integer.MIN_VALUE,Math::max));//dizideki en yuksek sayi

        l.stream().sorted(Comparator.reverseOrder()).limit(1).forEach(metodLambda2::bosluklaYazdir);//sralayip, tersleyip ilk eleman=en buyuk
        l.stream().sorted().limit(1).forEach(metodLambda2::bosluklaYazdir);//siralayip ilk eleman =en kucuk
    }
